    ...
    try {
      ...
      const readFile = async (fileEntry) => {
        try {
          ...
          // Encontrar índices das colunas B (Basic Info) e C (Enter Content)
          const columnIndexB = headerRow.indexOf('Basic Info');
          const columnIndexC = headerRow.indexOf('Enter Content');
  
          // Verificar se os índices foram encontrados corretamente
          if (columnIndexB !== -1 && columnIndexC !== -1) {
            // Encontrar o valor na coluna B para comparar
            const searchValue = 'SL'; // Valor a ser procurado na coluna B
            for (let i = 1; i < rows.length; i++) {
              const columns = rows[i].split(';');
  
              // Verificar se o valor na coluna B corresponde ao que estamos procurando ('SL')
              if (columns[columnIndexB] === searchValue) {
                // Se encontrarmos o valor correspondente na coluna B, obtemos o valor na coluna C
                slValue = columns[columnIndexC];
                console.log(`Valor correspondente na coluna C para 'SL': ${slValue}`);
                break; // Encerramos o loop assim que encontramos a correspondência
              }
            }
          } else {
            console.log('Colunas não encontradas no cabeçalho.');
          }
        } catch (error) {
      ...
      console.log("File Path CSV:", csvFilePath);
